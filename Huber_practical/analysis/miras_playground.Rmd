...
# prepare data
BR16_rf = dplyr::select(umap_count_BR16, -c("U1", "U2", "collection_date", "cell_type", "origin", "mouse_model", "ctc_n", "n_cells", "sample_title"))

# make col to rownames
BR16_rf = tibble::column_to_rownames(BR16_rf, var = "sample_id")
...
ER_GO_day <- enrichGO(
  gene = i_scores_subset$ensembl_gene_id,
  keyType = "ENSEMBL",
  OrgDb = "org.Hs.eg.db",
...
# plot GO
ggplot(ER_GO_day %>% arrange(qvalue) %>% slice_head(n=15),
       aes(x = -log10(qvalue), y = reorder(Description, -log10(qvalue)), size = Count, colour = -log10(qvalue))) +
  geom_point() +
...
```{r}
deseq_rf <- inner_join(rf_importance, resdds_classic)

ggplot(deseq_rf,
       aes(x = Overall_importance, y = log2FoldChange)) +
  geom_point()
```
